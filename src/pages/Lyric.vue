...
import { getShowTrack } from '~/store';
...
onBeforeRouteUpdate((to, from, next) => {
  if (to.query.path !== from.query.path) {
    getShowTrack(to.query.path as string).then((track) => {
...
if (props.path) {
  getShowTrack(props.path as string).then((track) => {
    viewTrack.value = track;
  });
}
</script>
...
<template>
  ...
  >
    <div class="flex items-end bg-gray-800/60 backdrop-blur-2xl w-full h-full">
      <img
        :src="viewTrack?.picture_url()"
        :class="`ml-5 mb-5 w-72 rounded-lg ${viewTrack?.picture_url() ? '' : 'invisible'} ring-1 ring-white/10`"
      >
      <div class="grow">
        lyric
      </div>
